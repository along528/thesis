...
So while the $WWW$ process has never been observed, the SM building
blocks of this process are not new. The descriptions below thus
rely mostly on those in \cite{commins1983weak}, \cite{halzen2008quark},
...
breaking (EWSB), while keeping $\gamma$ massless. The simplest form of EWSB
...
\caption{Summary of the electric charge, $Q$ (in unites of $e$), 
and masses of the SM fermions. Masses 
are taken from the Particle Data Group \cite{PDG:2014} and 
are shown to the best precision available within their uncertainties.
...
QCD, EW, and EWSB interactions; these are discussed in \sec\ref{sec:theory_qcd} and
\sec\ref{sec:theory_ew}.
The interactions, each with their own coupling parameters,
can be used (like the pieces of a puzzle)
...
using perturbation theory. For instance, say that 
a given fundamental interaction has a coupling 
parameter, $0 < \alpha < 1$. 
Since $0 < \alpha < 1$, terms with higher powers of $\alpha$ are suppressed
and can be ignored if one is willing to sacrifice some precision. 
The simplest ``tree-level'' interactions
...
theory being non-Abelian is that the gluons are also colored, 
though not in the same way as the quarks.
...
\unitlength = 0.4mm %necessary for feynmp
\begin{figure}[ht]
\centering
\vspace{5 mm}
...
quarks.
These interactions can be identified with the Feynman diagrams in
...
three different colors, indexed by $c$. There is also an implied
sum over the quark flavors. The gluon fields are represented
by $G_{\mu}^a$, where there is also an implied sum over the eight
different gluon fields, indexed by $a$. 
The gluon fields are four-vectors of the Lorentz group
and thus undergo Lorentz transformations as indicated by their Lorentz index, $\mu$.
The $T_a$ are the generators of \suthree, represented by the eight Gell-Mann matrices.
...
result in the peculiar property that $g_s$ decreases with increasing energy 
...
as free particles. On the other hand, 
...
At the LHC, hadronization frequently occurs
...
be probed, although indirectly. 
Indeed, in the high energy proton-proton collisions of the LHC, 
...
($pp \to Z\ \to l^+l^-$ or $pp\to Z\to q^+q^-$)
a natural scale to choose would be $\mu_R = \mu_F = m_Z$.},
...
characterized by  two up quarks and a down quark.
This translates into the requirements 
\begin{equation}
...
\begin{equation}
\int \mathrm{dx} (f_i(x,~\mu_R^2,~\mu_F^2) - f_{\overline{i}}(x,~\mu_R^2,~\mu_F^2) ) = 0, ~~\textrm{where}~  i\neq d,u.
\end{equation}
...
The PDFs can then be used along with the partonic cross-section for the 
given process,
$\hat{\sigma}$, to determine an overall cross-section for its production
in proton-proton scattering.
To achieve this, 
the PDFs are convoluted with the partonic
...
two incoming protons with partons indexed by $i$ and $j$, respectively.
...
The peaks in the $f_u$ and $f_d$ distributions are due to the valence
...
collider experiments. There isn't one perfect way 
to determine the PDFs.  As a result there are many different
PDF sets available to choose from, with updates being churned
out regularly by different physics groups. An example 
for one particular PDF set is shown using two different
...
When making predictions, one usually compares multiple
...
\begin{figure}[ht]
\centering
\includegraphics[width=.495\textwidth]{figures/theory/QCDKFac_Separate_WPLUS.eps}
\includegraphics[width=.495\textwidth]{figures/theory/QCDKFac_Separate_WMINUS.eps}
\caption{Plot of 
...
\end{figure}
%parton luminosity dropoff


For this thesis, three different PDF sets are compared for the signal 
...
during Run-I at HERA, vector boson production and 
...
with an implied sum over the fermion fields denoted as $\psi$.
...
electromagnetism, $Q$, 
and the charge of the third component of weak \sutwo, $T^3$, by 
\begin{equation}
Q=T^3+Y/2.
\label{eq:ewcharges}
\end{equation}
...
left-handed fermion isospin doublets, $L$,  and the right-handed
fermion isospin singlets, $R$, which are described in more detail below.
...
\end{equation}
Meanwhile, the right-handed fermion fields are treated as isospin singlets, $R$, 
...
of \fig\ref{fig:feyn_wcoupling} and 
it is the presence of these interactions that allow the $W$ 
to be produced directly at a hadron collider.
...
The strength of the weak interactions is suppressed at energies 
...
\end{fmffile}
\vspace{2 mm}
\caption{Feynman diagrams of EW QGC couplings}
...
by relations like those above, they are not all known \emph{a priori} 
...
at the SPS by observing its decay to an electron %abbreviation?
and an electron neutrino \cite{ARNISON1983103}.
...
sum to 100\% separately for the $W$ and $Z$ bosons within their uncertainties.}
...
The $Z$ boson was also discovered  at the SPS shortly after the 
$W$ \cite{ua1zobs}.
...
show the current measured value of the Higgs mass to be 
...
\begin{fmffile}{feynwwwprod4}
		\begin{fmfgraph*}(80,50)
			...
			\fmf{fermion,label=$q'$}{v1,v2}
			\fmf{fermion}{v2,i2}
			\fmf{photon,label=$Z/\gamma^{*}$,label.side=left}{v2,v3}
			\fmf{photon}{v1,o1}
			\fmf{photon}{v3,o2}
			\fmf{photon}{v3,o3}
			\fmflabel{$q$}{i1}
...
up-type quark (anti-quark) and down-type anti-quark (quark). 
In this case, the charge of the initial state quarks is assumed
to sum to +1, leading to the same charge in the final state.
...
distribution of $m_{\Wp\Wm}$ taken from the simulation of \www~events
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.6\textwidth]{figures/2l2j/mWW-parton.pdf}
\caption{ Invariant mass distribution of two opposite-sign $W$ bosons 
in \www~events at truth level. The Higgs mass peak is clearly 
visible around 125~GeV.}
\label{fig:mww_higgs}
\end{figure}



In principle, the LO partonic cross-section for the exclusive processes, 
...
and semi-leptonic channels is presented in Chapter \ref{sec:combination}.
...
The leading $n=6$ terms predict new anomalous triple gauge coupling
(aTGC) and anomalous quartic gauge coupling (aQGC)
interactions, while the sub-leading $n=8$ terms
predict only new aQGC interactions. 
Predictions of aTGC interactions have been studied in detail at 
LEP, the Tevatron, and the LHC,  though none have been observed~\cite{PDG:2014}.
...
them in the ratio with the couplings:
$f_{S,0}/\Lambda^4$ and $f_{S,1}/\Lambda^4$.
Above $\Lambda$ the EFT breaks down and can lead to unitarity violation.
In particular, for the operators with dimension $n=8$, 
...
relevant for aQGCs in the context of $WWW$ production. Thus, if we 
...
\begin{figure}[htp]
  ...
  \caption{The unitarized and non-unitarized differential cross
    ...
    used for unitarization.  The choice of parameters
    here serves merely as a demonstration.}
\label{fig:unitarity_example}
\end{figure}


In order to unitarize the EFT predictions we choose to 
apply an energy dependent form-factor
to the aQGC parameters 
according to the approach from \cite{Eboli:2003nq}.
The form-factor, $f(s)$, looks like
\begin{equation}
...
introduce a model dependence into the EFT. The effect of the
form-factor is to reduce the differential cross-section as a
...
The unitarization
scheme has been employed recently by 
ATLAS in \cite{PhysRevLett.115.031802}.
...
A variety of measurements sensitive to SM QGC interactions have been
...
and to $WWWW$ at the LHC \cite{PhysRevLett.113.141803,PhysRevLett.114.051801}.  No measurements, however, have been performed
looking at the $WWW$ production process directly.
...
Non-unitarized limits on the $f_{S,0}$ vs $f_{S,1}$ aQGC 
parameters have been performed by CMS in \cite{PhysRevLett.114.051801}.
