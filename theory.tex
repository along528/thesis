...
The strength of the weak interactions is supressed by the massive
...
gauge bosons by adding new fields (?). The simplest explanation
is the so-called Higgs (brout-engletrt?) (cite) mechanism.
...
introduced Higgs field, $\phi$, with the gauge bosons; 
...
breaking in the Higgs mechansim;
and the third line shows the Yukawa (cite) interactions 
...
are complex isopsin doublets of four scalar fields:
\begin{equation}
\phi = \frac{1}{\sqrt{2}}
\begin{pmatrix} 
\phi_1 + i \phi_2 \\
...
vacuum fluctuations of the Higgs field about this minimum.
...
\begin{equation}
\frac{vg^2}{8} h W^+_{\mu}W^{-\mu} + \frac{g^2}{4} h^2W^{+}_{\mu}W^{-}_{\mu}
